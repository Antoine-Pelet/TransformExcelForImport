...
/**
 * \file        class/transformexcelforimport.class.php
 * \ingroup     transformexcelforimport
 * \brief       This file is a CRUD class file for Transformexcelforimport (Create/Read/Update/Delete)
 */
...
/**
 * Class for Transformexcelforimport
 */
class Transformexcelforimport extends CommonObject
{
	/**
	 * @var string 	ID of module.
	 */
	public $module = 'transformexcelforimport';

	/**
	 * @var string 	ID to identify managed object.
	 */
	public $element = 'transformexcelforimport';

	/**
	 * @var string 	Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'transformexcelforimport_transformexcelforimport';

	/**
	 * @var string 	If permission must be checkec with hasRight('transformexcelforimport', 'read') and not hasright('mymodyle', 'transformexcelforimport', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'transformexcelforimport';

	/**
	 * @var string 	String with name of icon for transformexcelforimport. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'transformexcelforimport@transformexcelforimport' if picto is file 'img/object_transformexcelforimport.png'.
	 */
	...
	//  */
	// public $table_element_line = 'transformexcelforimport_transformexcelforimportline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_transformexcelforimport';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'Transformexcelforimportline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'Transformexcelforimport', 'fk_element'=>'fk_transformexcelforimport'));
	...
	//  */
	// protected $childtablesoncascade = array('transformexcelforimport_transformexcelforimportdet');

	// /**
	//  * @var TransformexcelforimportLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('transformexcelforimport', 'transformexcelforimport', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('transformexcelforimport', 'transformexcelforimport', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('transformexcelforimport', 'transformexcelforimport_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'transformexcelforimport/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'transformexcelforimport/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filepath = 'transformexcelforimport/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'transformexcelforimport/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->transformexcelforimport->dir_output.'/transformexcelforimport/'.$oldref;
				$dirdest = $conf->transformexcelforimport->dir_output.'/transformexcelforimport/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->transformexcelforimport->dir_output.'/transformexcelforimport/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('transformexcelforimport','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('transformexcelforimport','transformexcelforimport_advance','validate'))))
		 {
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'TRANSFORMEXCELFORIMPORT_MYOBJECT_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'TRANSFORMEXCELFORIMPORT_MYOBJECT_CANCEL');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'TRANSFORMEXCELFORIMPORT_MYOBJECT_REOPEN');
	}
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowTransformexcelforimport")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("Transformexcelforimport").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/transformexcelforimport/transformexcelforimport_card.php', 1).'?id='.$this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowTransformexcelforimport");
	...
	public function getLinesArray()
	{
		...
		$objectline = new TransformexcelforimportLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_transformexcelforimport:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('TRANSFORMEXCELFORIMPORT_MYOBJECT_ADDON')) {
			$conf->global->TRANSFORMEXCELFORIMPORT_MYOBJECT_ADDON = 'mod_transformexcelforimport_standard';
		}

		if (getDolGlobalString('TRANSFORMEXCELFORIMPORT_MYOBJECT_ADDON')) {
			$mybool = false;

			$file = getDolGlobalString('TRANSFORMEXCELFORIMPORT_MYOBJECT_ADDON').".php";
			$classname = getDolGlobalString('TRANSFORMEXCELFORIMPORT_MYOBJECT_ADDON');

			// Include file with class
			$dirmodels = array_merge(array('/'), (array) $conf->modules_parts['models']);
			foreach ($dirmodels as $reldir) {
				$dir = dol_buildpath($reldir."core/modules/transformexcelforimport/");
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefTransformexcelforimport $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_transformexcelforimport';
		...
		$modelpath = "core/modules/transformexcelforimport/doc/";
...
/**
 * Class TransformexcelforimportLine. You can also remove this and generate a CRUD class for lines objects.
 */
class TransformexcelforimportLine extends CommonObjectLine
{
	// To complete with content of an object TransformexcelforimportLine
	// We should have a field rowid, fk_transformexcelforimport and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'transformexcelforimport'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_transformexcelforimport'
